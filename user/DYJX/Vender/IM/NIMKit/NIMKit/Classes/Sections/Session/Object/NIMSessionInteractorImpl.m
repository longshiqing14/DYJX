...
{
    RCIMMessage *message = messages.firstObject;
...
{
    NSMutableArray *models = [[NSMutableArray alloc] init];
    for (RCIMMessage *message in messages) {
...
{
    if ([message isKindOfClass:[RCIMMessage class]]) {
...
- (NSMutableArray *)findRemainAudioMessages:(RCIMMessage *)message
{
    if (message.isPlayed || [message.extraDic[@"FromId"] isEqualToString:[UserManager shared].getUserModel.UserID]) {
    ...
    [self.dataSource.items enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[NIMMessageModel class]]) {
            ...
            BOOL isFromMe = [model.message.extraDic[@"FromId"] isEqualToString:[UserManager shared].getUserModel.UserID];
            if ([model.message.messageUId isEqualToString:message.messageUId])
            {
                *stop = YES;
            }
            else if ([NSString stringWithFormat:@"%@",model.message.extraDic[@"MessageType"]].integerValue == 2 && !isFromMe && !model.message.isPlayed)
            {
