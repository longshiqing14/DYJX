...
- (NSInteger)indexAtModelArray:(NIMMessageModel *)model
{
    ...
    if (![_msgIdDict objectForKey:model.message.messageUId]) {
        return index;
    }
...
{
    return [_msgIdDict objectForKey:model.message.messageUId] != nil;
}
...
- (NSArray*)deleteMessageModel:(NIMMessageModel *)msgModel
{
    ...
    if (delMsgIndex > -1) {
        ...
        [_msgIdDict removeObjectForKey:msgModel.message.messageUId];
...
- (NSArray<NSIndexPath *> *)deleteModels:(NSRange)range
{
    ...
    for (NIMMessageModel *model in models) {
        ...
        if (delMsgIndex > -1) {
            ...
            [_msgIdDict removeObjectForKey:model.message.messageUId];
...
- (void)insertMessage:(NIMMessage *)message{
    ...
    [self.msgIdDict setObject:model forKey:model.message.messageUId];
}
