...
@interface DYLocationViewController ()<UITableViewDelegate,UITableViewDataSource,QMapViewDelegate,UISearchResultsUpdating,UISearchControllerDelegate,QMSSearchDelegate>
...
- (void)setMapView
{
    ...
    if (self.isSubmit) {
        ...
        self.mapView = [[QMapView alloc] initWithFrame:CGRectMake(0, 64, self.view.bounds.size.width, kScreenHeight - 64)];
    }
...
- (void)setMainTableView
{
    if (self.isSubmit) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.mapView.frame), self.view.bounds.size.width, 0) style:UITableViewStylePlain];
    }
    ...
    if (self.isSubmit) {
        ...
        self.imageViewAnntation.image = [UIImage imageNamed:@""];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((self.centerPoint.coordinate.latitude == center.latitude && self.centerPoint.coordinate.longitude == center.longitude) || (self.centerPoint.coordinate.latitude == 0 && self.centerPoint.coordinate.longitude == 0 && indexPath.row == 0)) {
        img.hidden = NO;
    }
    else {
        img.hidden = YES;
