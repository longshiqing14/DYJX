...
#import "JXAlreadyUsedViewModel.h"
#import "JXShopOpenCell.h"
#import "JXShopCell.h"
static NSString *cellHeat=@"cellHeat";
static NSString *cellID=@"cellID";
static NSString *cellID1=@"cellID1";
@interface JXAlreadyUsedViewController ()
...
@property(nonatomic, strong)JXAlreadyUsedViewModel *ViewModel;
@property (nonatomic ,strong)NSMutableArray<NSNumber*> *recordClickedArray;
@end

@implementation JXAlreadyUsedViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [_alreadyTableView registerNib:[UINib nibWithNibName:@"JXShopCell" bundle:nil] forCellReuseIdentifier:cellID];
    [_alreadyTableView registerNib:[UINib nibWithNibName:@"JXShopOpenCell" bundle:nil] forCellReuseIdentifier:cellID1];
    [_alreadyTableView registerNib:[UINib nibWithNibName:@"JXMyAleadyUsedHeatView" bundle:nil] forHeaderFooterViewReuseIdentifier:cellHeat];
    
    [self addHeaderFooter];
    self.alreadyTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.alreadyTableView setSeparatorColor:[UIColor clearColor]];
    [self.alreadyTableView.mj_header beginRefreshing];
    self.view.backgroundColor = [UIColor colorWithHexString:@"#F4F4F4"];
}

- (void)addHeaderFooter{
    WeakSelf;
    self.alreadyTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [self.ViewModel refreshDataSuccess:^(BOOL isLastPage, BOOL isHaveData) {
            if (isHaveData) {
                [weakSelf.alreadyTableView removeTipViewFromSelf];
            }else {
                [weakSelf.alreadyTableView showNoDataCustomTipViewWithText:@"您暂无相关优惠券哦～" imageString:@"no_search" WithInset:80.];
            }
            [weakSelf.alreadyTableView.mj_header endRefreshing];
...
//    self.alreadyTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
...
//            [weakSelf updateRecord:weakSelf.listArray];
//            if (isLastPage) {
//                [weakSelf.alreadyTableView.mj_footer endRefreshingWithNoMoreData];
//            }else {
//                [weakSelf.alreadyTableView.mj_footer endRefreshing];
//            }
//            [weakSelf.alreadyTableView reloadData];
//        } failed:^(NSString *errorMsg) {
//            [weakSelf.alreadyTableView.mj_footer endRefreshing];
...
-(void)Refresh:(NSInteger)refreshIndex{
    ...
    [_alreadyTableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic]; }
...
- (JXAlreadyUsedViewModel *)ViewModel{
    if (!_ViewModel) {
        _ViewModel = [[JXAlreadyUsedViewModel alloc]init];
    }
    return _ViewModel;
}
