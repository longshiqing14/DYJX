...
- (void)setFrame:(CGRect)frame
{
    ...
    _increaseBtn.frame = CGRectMake(height+(width - height * 2.8), 0, height, height);
    _textField.frame = CGRectMake(height, 0, width - height * 2.8, height);
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_decreaseBtn.bounds byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerTopLeft cornerRadii:CGSizeMake(10, 10)];
    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
    maskLayer.frame = _decreaseBtn.bounds;
    maskLayer.path = maskPath.CGPath;
    _decreaseBtn.layer.mask = maskLayer;
}
...
- (void)increase
{
    ...
    if (number<=_maxnumber) {
        _increaseBtn.userInteractionEnabled=YES;
        _decreaseBtn.userInteractionEnabled=YES;
...
- (void)decrease
{
    ...
    else
    {
        ...
        NSLog(@"数量不能小于1");
        _decreaseBtn.userInteractionEnabled=NO;
        if (self.PPClockSmall) {
            self.PPClockSmall();
        }
    }
}
