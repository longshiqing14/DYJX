//
//  JXGoodsDetailCouponViewModel.m
//  user
//
...
- (void)getMyExtensoinListWithPageNumer:(NSInteger)pageNumber Success:(void(^)(BOOL isLastPage,BOOL doHaveData))success failed:(void(^)(NSString *errorMsg))fail{
    ...
    NSMutableDictionary * dict = [XYBestRequest requestAllDataWithApi_ID:kJXAPI_user_productDetailCard request_data:self.requestDic];
    ...
    [XYNetWorking XYPOST:@"" params:dict success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            if ([[responseObject objectForKey:RETURN_CODE_] isEqualToString:ERROR_NUM_STRING_] ) {
                ...
                if (pageNumber == 1) {
                    JXCouponCardBaseModel *model = [JXCouponCardBaseModel modelWithJSON:[responseObject valueForKey:RETURN_DATA_]];
                    [self.dataListArry removeAllObjects];
                    [self.dataListArry addObjectsFromArray:model.cardArrs];
                }else{
                    JXCouponCardBaseModel *model = [JXCouponCardBaseModel modelWithJSON:[responseObject valueForKey:RETURN_DATA_]];
                    [self.dataListArry addObjectsFromArray:model.cardArrs];
                }
...
- (void)receiveCouponWithActiveId:(NSString*)activeId Success:(void(^)(BOOL isLastPage,NSString* desc))success failed:(void(^)(NSString *errorMsg))fail{
    ...
    [self.requestDic setValue:activeId forKey:@"activeId"];
    
    NSMutableDictionary * dict = [XYBestRequest requestAllDataWithApi_ID:kJXAPI_user_productAddSalePromotion request_data:self.requestDic];
    ...
    [XYNetWorking XYPOST:@"" params:dict success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            if ([[responseObject objectForKey:RETURN_CODE_] isEqualToString:ERROR_NUM_STRING_] ) {
                ...
                //                if (pageNumber == 1) {
                                    JXReceiveCouponModel *model = [JXReceiveCouponModel modelWithJSON:[responseObject valueForKey:RETURN_DATA_]];
                //                    [self.dataListArry removeAllObjects];
