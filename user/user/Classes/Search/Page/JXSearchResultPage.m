...
//请求后台数据
   ...
   [weakSelf.searchResultCollectionView.mj_footer resetNoMoreData];
    [weakSelf.searchResultViewModel refreshDataSuccess:^(BOOL isLastPage,BOOL doHaveData) {
        ...
        if (doHaveData) {
            ...
            [self.searchResultCollectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:0] atScrollPosition:UICollectionViewScrollPositionTop animated:NO];
        }else {
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:self.styleView];
    ...
    [rightBarButton addTarget:self action:@selector(Back:) forControlEvents:UIControlEventTouchUpInside];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isList) {
        ...
        tableCell.sellingPointLable.text = [self.searchResultViewModel sellingPointText:indexPath];
        tableCell.goodsPriceLabel.text = [NSString stringWithFormat:@"¥ %@", [self.searchResultViewModel goodsPrice:indexPath]];
        NSLog(@"%@",[self.searchResultViewModel goodsLable:indexPath]);
...
- (UICollectionViewFlowLayout *)cubeLayout
{
    if (!_cubeLayout) {
        ...
        _cubeLayout.itemSize = CGSizeMake((kScreenWidth - 3) / 2, 350);//AutoSize(295));
